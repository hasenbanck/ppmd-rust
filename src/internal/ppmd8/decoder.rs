...
pub unsafe fn decode_symbol<R: Read>(
    p: *mut PPMd8<RangeDecoder<R>>,
    ...
    if (*(*p).min_context).num_stats as std::ffi::c_int != 0 as std::ffi::c_int {
        ...
        count = (*p).rc.get_threshold(summFreq);
        hiCnt = count;
    ...
    loop {
        ...
        let freqSum2 = (*p).rc.correct_sum_range(freqSum);

        count_0 = (*p).rc.get_threshold(freqSum2);
        ...
        (*p).rc.decode(hiCnt_0, freqSum2.wrapping_sub(hiCnt_0));
