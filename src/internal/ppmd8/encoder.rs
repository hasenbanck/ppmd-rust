    ...
    pub unsafe fn encode_symbol(&mut self, symbol: std::ffi::c_int) -> std::io::Result<()> {
        ...
        if self.min_context.as_ref().num_stats as std::ffi::c_int != 0 as std::ffi::c_int {
            ...
            if s.as_ref().symbol as std::ffi::c_int == symbol {
                self.rc.encode_final(0, s.as_ref().freq as u32, summFreq)?;
                self.found_state = s;
                self.update1_0();
                return Ok(());
            }
            ...
            loop {
                ...
                if s.as_ref().symbol as std::ffi::c_int == symbol {
                    self.rc
                        .encode_final(sum, s.as_ref().freq as u32, summFreq)?;
                    self.found_state = s;
                    self.update1();
                    return Ok(());
                }
        ...
        loop {
            ...
            self.min_context = mc;
            let mut see = self.make_esc_freq(numMasked, &mut escFreq);
            let mut s_1 = (self.memory_ptr)
                .offset(self.min_context.as_ref().union4.stats as isize)
                .cast::<State>();
            sum_0 = 0 as std::ffi::c_int as u32;
            i_0 = (self.min_context.as_ref().num_stats as std::ffi::c_uint)
