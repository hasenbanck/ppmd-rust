...
use crate::native::ppmd7::*;
use crate::native::*;
use crate::*;

pub unsafe fn Ppmd7z_RangeDec_Init(mut p: *mut CPpmd7_RangeDec) -> BoolInt {
    ...
    while i < 4 as std::ffi::c_int as std::ffi::c_uint {
        (*p).Code = (*p).Code << 8 as std::ffi::c_int
            | ((*(*p).Stream).Read).expect("non-null function pointer")((*p).Stream) as UInt32;
        i = i.wrapping_add(1);
        i;
    }
...
#[inline(always)]
unsafe fn Ppmd7z_RD_Decode(mut p: *mut CPpmd7, mut start: UInt32, mut size: UInt32) {
...
pub unsafe fn Ppmd7z_DecodeSymbol(mut p: *mut CPpmd7) -> std::ffi::c_int {
    ...
    if (*(*p).MinContext).NumStats as std::ffi::c_int != 1 as std::ffi::c_int {
        ...
        let summFreq: UInt32 = (*(*p).MinContext).Union2.SummFreq as UInt32;
        ...
        if (count as Int32) < 0 as std::ffi::c_int {
            let mut sym: Byte = 0;
            Ppmd7z_RD_Decode(p, 0 as std::ffi::c_int as UInt32, (*s).Freq as UInt32);
            if (*p).rc.dec.Range < (1 as std::ffi::c_int as UInt32) << 24 as std::ffi::c_int {
        ...
        *(charMask.as_mut_ptr() as *mut Byte).offset((*s).Symbol as isize) =
        ...
        let mut size0: UInt32 = ((*p).rc.dec.Range >> 14 as std::ffi::c_int) * pr;
        pr = pr.wrapping_sub(
            pr.wrapping_add(
